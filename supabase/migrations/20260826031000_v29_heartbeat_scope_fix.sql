-- V29 correction: options remain visible to authorized managers while advisor logs remain self-only.
...
begin
  ...
  with scoped_teams as (
    ...
    select p.id, p.display_name, p.primary_team_id from public.profiles p where p.is_active and p.primary_team_id in (select id from scoped_teams)
  ), log_rows as (
